...
const storedSidebarWidth = Number(localStorage.getItem("gdou.sidebarWidth"));
...
function readCollapsedProjects(): string[] {
  try { const raw = localStorage.getItem("gdou.collapsedProjects"); return raw ? (JSON.parse(raw) as string[]) : []; } catch { return []; }
}
...
const inspectorWidth = ref(Math.min(720, Math.max(340, Number(localStorage.getItem("gdou.inspectorWidth")) || 390)));
...
// 专家是内核的方法论文件（内置/用户/项目三级），列表在 refreshIndex 时拉取。
// 会话创建时把选中的专家带给桥端，桥端按配方装配（专家只能收窄工具集）。
const experts = ref<ExpertSummary[]>([]);
const selectedExpert = ref<string | null>(null);
const expertMenuOpen = ref(false);
...
const statusBarVisible = ref(localStorage.getItem("gdou.statusBarVisible") !== "false");
const webviewZoom = ref(Number(localStorage.getItem("gdou.webviewZoom")) || 1);
...
async function refreshIndex(loadHistory = false) {
 ...
 return; }
  ...
  // 专家列表独立拉取：失败不该拖垮整页（内核里专家只是可选的收窄层）。
  try { experts.value = await listExperts(); } catch { /* 保持上一次的列表 */ }
...
async function submitTask(
  ...
  sending.value = true;
  try {
    const created = await createSession(project, selectedExpert.value);
    const sessionId = created.session_id;
    // 先把新会话放入本地索引，避免等待下一次 session.list 才能渲染会话区。
    insertProvisionalSession(sessionId, displayText.trim() || trimmed, project);
    // 新建会话时记录专家：会话头据此显示「为什么工具变少了」的收窄提示。
    if (created.expert) {
      sessions.value = sessions.value.map((item) => item.session_id === sessionId ? { ...item, expert: created.expert } : item);
    }
    const view = ensureSessionView(sessionId);
    view.timeline = new Map();
    view.activeRunId = null;
...
async function chooseTask(id: string) {
  ...
  taskSearchOpen.value = false;
  try {
    const stored = JSON.parse(localStorage.getItem("gdou.unreadSessions") ?? "[]");
    if (Array.isArray(stored) && stored.includes(id)) {
      localStorage.setItem("gdou.unreadSessions", JSON.stringify(stored.filter((item) => item !== id)));
    }
  } catch {
    // Ignore unavailable localStorage in embedded/private webviews.
  }
  document.dispatchEvent(new CustomEvent("gdou:session-unread-change", {
...
const onNativeFileDrop = createComposerFileDropHandler({
  ...
  containsPoint: (x, y) => !fileDropDisposed && !!document.elementFromPoint(x, y)?.closest(".gdou-composer"),
...
/** 当前会话绑定的专家名（用于会话头徽标），无专家返回 null。 */
const activeExpertName = computed(() => {
  const expertId = active.value?.expert ?? null;
  if (!expertId) return null;
  return experts.value.find((item) => item.id === expertId)?.name ?? expertId;
});
const selectedExpertName = computed(() => {
  if (!selectedExpert.value) return null;
  return experts.value.find((item) => item.id === selectedExpert.value)?.name ?? selectedExpert.value;
});
function toggleExpertMenu() {
  if (!experts.value.length) return;
  closeLauncherMenus();
  expertMenuOpen.value = !expertMenuOpen.value;
}
...
function closeExpertMenuOnOutside(event: PointerEvent) {
  ...
  if (!target) return;
  if (target instanceof Element && target.closest(".expert-picker")) return;
  expertMenuOpen.value = false;
}
...
function toggleStatusBar() {
  ...
  localStorage.setItem("gdou.statusBarVisible", String(statusBarVisible.value));
}
async function applyZoom(next: number) {
  const zoom = Math.min(2, Math.max(.6, Math.round(next * 10) / 10));
  webviewZoom.value = zoom;
  localStorage.setItem("gdou.webviewZoom", String(zoom));
...
async function openProjectHomepage() {
  ...
  await openUrl("https://github.com/NaLanBoO0/Gdou-agent");
}
async function openLicense() {
  const { openUrl } = await import("@tauri-apps/plugin-opener");
  await openUrl("https://github.com/NaLanBoO0/Gdou-agent/blob/main/LICENSE");
}
...
function resizeSidebarWithKeyboard(event: KeyboardEvent) {
  ...
  localStorage.setItem("gdou.sidebarWidth", String(sidebarWidth.value));
}
...
onBeforeUnmount(() => {
  ...
  window.removeEventListener("gdou:open-in-app-browser", onOpenInAppBrowser);
  window.removeEventListener("gdou:open-file", onOpenFileLink);
  window.removeEventListener("gdou:inject-element", onInjectElement);
...
<template>
  ...
  >
    ...
    <header class="gdou-titlebar" :class="{ 'is-macos': isMacOS }">
    ...
    <div class="sidebar-viewport">
      <aside id="primary-navigation" class="gdou-sidebar agent-sidebar">
    ...
    <main class="gdou-main work-active">
      <div v-show="page === 'work'" class="work-page-host">
          ...
          <div class="work-layout" :class="{ 'no-inspector': !inspectorOpen || !activeWorkspace, 'inspector-resizing': inspectorResizing }" :style="workLayoutStyle">
            <section class="task-canvas">
              ...
              <header class="work-header">
                ...
                <span v-if="activeExpertName" class="work-header__expert" :title="t('app.expertActiveOnSession', { name: activeExpertName })"><AppIcon name="Brain" :size="13" /><span>{{ activeExpertName }}</span></span>
        ...
        <section v-else class="landing-page task-launcher" :class="{ 'slash-open': slashMenuOpen }">
          <div class="launcher-content">
            ...
            <form class="gdou-composer landing-composer" :class="{ 'drag-over': isDragOver }" @submit.prevent="submit()" @dragenter="onDragEnter" @dragover="onDragOver" @dragleave="onDragLeave" @drop="onDrop">
              <SlashCommandMenu v-if="slashMenuOpen" :query="slashQuery ?? ''" :skills="composerSkills" :loading="skillQueryLoading" :error="skillQueryError" :connected="connected" :active-index="slashMenuActiveIndex" @activate="slashMenuActiveIndex = $event" @select="chooseSkill" />
              <div class="composer-input-shell" :class="{ 'has-plugin-tags': insertedPlugins.length, 'has-skill-tag': selectedSkill }">
                <div v-if="attachedFiles.length" class="attachment-strip"><AttachmentChip v-for="(file, index) in attachedFiles" :key="file.path" :file="file" @remove="removeAttachment(index)" /></div>
                <p v-if="attachedFiles.some((file) => file.kind === 'image')" class="image-processing-mode" aria-live="polite">{{ imageProcessingLabel }}</p>
