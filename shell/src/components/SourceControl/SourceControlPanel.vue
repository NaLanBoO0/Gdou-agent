...
import { changeDiff, commitChanges, discardChanges, gitHistory, listChanges, stageChanges, unstageChanges, workspaceStatus, type ChangeSummary, type GitCommitEntry, type WorkspaceStatus } from "../../services/gdou-runtime";
...
<template>
  <section class="source-control-page">
    <header class="source-control-header"><button type="button" class="source-control-back" aria-label="返回会话" title="返回会话" @click="emit('close')"><AppIcon name="ArrowLeft" :size="17" /></button><div class="source-control-title"><AppIcon name="GitBranch" :size="18" /><div><h1>源代码管理</h1><p>{{ workspaceName }}<span>{{ workspacePath }}</span></p></div></div><button type="button" class="source-control-refresh" aria-label="刷新源代码管理" title="刷新" :disabled="loading || graphLoading" @click="refreshCurrentView"><AppIcon name="RefreshCw" :size="16" :class="{ spin: loading || graphLoading }" /></button></header>
    <div class="source-control-branch"><AppIcon name="GitBranch" :size="14" /><b>{{ status?.branch || "未检测到分支" }}</b><small>{{ status?.is_git_repository ? "Git 仓库" : "非 Git 仓库" }}</small></div>
    ...
    <label class="source-control-search"><AppIcon name="Search" :size="14" /><input v-model="query" placeholder="筛选更改" aria-label="筛选更改" /></label>
