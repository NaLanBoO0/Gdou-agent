...
const artifacts = computed<Artifact[]>(() => {
  ...
  const isIgnoredPath = (p: string) => {
    ...
    const ignoredDirs = new Set([
      ...
      "build", "dist", ".cache", ".gdou", ".pytest_cache", ".mypy_cache",
...
/** 把选中元素组装成结构化文本，经 gdou:inject-element 事件追加到对话输入框 */
function addElementToConversation(tab: BrowserTab) {
  ...
  window.dispatchEvent(new CustomEvent("gdou:inject-element", { detail: { text: lines.join("\n") } }));
