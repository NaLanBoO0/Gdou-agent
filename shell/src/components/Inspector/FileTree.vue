...
const treeError = ref("");
const treeWidth = ref(Number(localStorage.getItem("gdou.treeWidth")) || 200);
const treeCollapsed = ref(localStorage.getItem("gdou.treeCollapsed") === "1");
...
function startTreeDrag(event: MouseEvent) {
  ...
  const onMove = (moveEvent: MouseEvent) => {
    ...
    localStorage.setItem("gdou.treeWidth", String(next));
  };
  const onUp = () => {
    dragCleanup?.();
    dragCleanup = null;
...
function toggleTreeCollapse() {
  ...
  localStorage.setItem("gdou.treeCollapsed", treeCollapsed.value ? "1" : "0");
}
