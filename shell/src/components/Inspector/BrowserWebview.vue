...
function buildPickerScript(useNative: boolean) {
  return String.raw`
(() => {
  if (window.__gdouPicker) {
    window.__gdouPicker.active ? window.__gdouPicker.stop() : window.__gdouPicker.start();
    return;
  }
  ...
  function onClick(e) {
    ...
    if (el && el.tagName) {
      ...
      setTimeout(() => window.__gdouPicker.stop(), 350);
    } else {
      window.__gdouPicker.stop();
    }
  }
  ...
  window.__gdouPicker = { start, stop, get active() { return active; } };
  start();
})()
`;
}
...
// Rust WebMessageReceived 桥 → gdou:menu-action 事件回传主窗口。
...
/** Rust 原生桥广播的事件监听句柄（gdou:element-picked / gdou:menu-action） */
