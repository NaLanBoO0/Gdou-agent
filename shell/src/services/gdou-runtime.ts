...
const PLUGIN_PROTOCOL_ERROR = "本地服务版本过旧，不支持插件市场。请完全退出旧的 Gdouwork 服务 后重新打开客户端。";
const GIT_PROTOCOL_ERROR = "本地服务版本过旧，不支持当前 Git 功能。请完全退出旧的 Gdouwork 服务 后重新打开客户端。";
...
async function waitForDaemon(): Promise<void> {
  // Browser mode: there is no Tauri daemon to start — the gdou bridge is already
  // listening on 7438, and the client connects to it over WebSocket directly.
  if (!IS_TAURI) return;
